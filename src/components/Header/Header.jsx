import './Header.css';
import React, { useContext, useRef, useState } from 'react';
import { Link, NavLink } from 'react-router-dom';
import { GiCommercialAirplane } from "react-icons/gi";
import { FaHotel, FaTrain } from "react-icons/fa";
import { MyContext } from '../Context/Context';

const Header= ()=>{
    const myContext = useContext(MyContext);
    const ref = useRef('');
    window.addEventListener('scroll', ()=>{
        // console.log(ref.current.style);
    })
    return(
        <div className='header'>
            <nav className='header-contents'>
                <Link to={'/'}><img className='logo' src={'../../MakeMyTrip logo white.png'} alt="MakeMyTrip" /></Link>
                {
                    myContext.loggedIn ?
                    <Link to={'/dashboard'} className='user-profile'>
                        <img className='user-image' src='data:image/png;base64,' alt='profile img' />
                        <p className='user-name'>{myContext.currUser.name}</p>
                    </Link>
                    :
                    <Link to={'/login'} className='user-profile'>
                        <img className='user-image' src='data:image/png;base64,' alt='profile img' />
                        <p className='user-name'>Login</p>
                    </Link>
                }
            </nav>
            <div ref={ref} className='pages-link'>
                <NavLink className={'page-link'} to={'/flights'}><GiCommercialAirplane /> Flights</NavLink>
                <NavLink className={'page-link'} to={'/hotels'}><FaHotel/> Hotels</NavLink>
                <NavLink className={'page-link'} to={'/trains'}><FaTrain/> Trains</NavLink>
            </div>
        </div>
    )
}
export default Header;